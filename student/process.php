...
if($_SERVER['REQUEST_METHOD']=="POST"){
  ...
  if(empty($_POST['phone'])){
    ...
    //check for any error
        if($isError === false){
            //run sql here...
            $studentsId = rand(9087, 34576);
            $sql= $conn->query("INSERT INTO student SET studentsId='$studentsId', name='$name', email='$email', phone='$phone', password='$password', cpass='$cpass' ");
            if($sql){
        ...
        if (isset($_POST['student_login'])) {
           ...
           $sql = $conn->query("SELECT * FROM `student` WHERE password='$password' AND email='$email' ");
          
               if($sql->num_rows>0){
                   //we have this record in our db
                   $row = $sql->fetch_assoc();
                       $_SESSION['student_login']=true;
                       $_SESSION['studentsId']= $row['studentsId'];
                       $_SESSION['email']= $row['email'];
                       $_SESSION['password']= $row['password'];
                      ?>
                       <script>alert('Welcome');
       window.location.replace('./student/home.php')</script>
       <?php
